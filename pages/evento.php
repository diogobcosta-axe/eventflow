...
// Lê o ID do evento da URL e valida
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
...
// Procura o evento com os dados da categoria e do organizador
$stmt = $db->prepare("
...
// Calcula ocupação e verifica inscrição do utilizador atual
...
<div class="event-detail">
    ...
    <?php else: ?>
    <div class="event-detail__img-placeholder"><?= e($ev['cat_icone']) ?></div>
    <?php endif; ?>
    ...
    <div class="event-detail__header fade-in">
        ...
        <div class="event-detail__meta-grid">
            <div class="meta-item">
                ...
                <?php if ($ev['data_fim']): ?>
                <div style="font-size:.8rem;color:var(--clr-muted)">ate <?= formatDate($ev['data_fim']) ?></div>
                <?php endif; ?>
            </div>
            <div class="meta-item">
            ...
            <div class="meta-item">
                ...
                <div class="meta-item__value"><?= $vagas_disp ?> disponiveis de <?= $ev['vagas'] ?></div>
            </div>
            <div class="meta-item">
        ...
        <div class="inscricao-box fade-in-2">
            ...
            <?php if ($ev['estado'] !== 'ativo'): ?>
                <div style="text-align:center;padding:16px;color:var(--clr-muted)">
                    Este evento esta <?= e($ev['estado']) ?> e nao aceita inscricoes.
                </div>
            ...
            <?php elseif ($inscricao && $inscricao['estado'] === 'confirmada'): ?>
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
                    <div>
                        <strong>Estas inscrito(a)!</strong>
                        <p style="font-size:.85rem;color:var(--clr-muted)">
                            Inscricao confirmada em <?= formatDate($inscricao['criado_em']) ?>.
                        </p>
                    </div>
                </div>
                <form method="POST" action="/pages/inscricao.php"
                      onsubmit="return confirm('Tens a certeza que queres cancelar a inscricao?')">
                    <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                    <input type="hidden" name="evento_id" value="<?= $ev['id'] ?>">
                    <input type="hidden" name="action" value="cancelar">
                    <button type="submit" class="btn btn--danger btn--sm">Cancelar inscricao</button>
                </form>

            <?php elseif ($inscricao && $inscricao['estado'] === 'cancelada'): ?>
                <p style="color:var(--clr-muted);margin-bottom:16px">Tinhas cancelado a inscricao neste evento.</p>
                <?php if ($vagas_disp > 0): ?>
                <form method="POST" action="/pages/inscricao.php">
                    <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                    <input type="hidden" name="evento_id" value="<?= $ev['id'] ?>">
                    <input type="hidden" name="action" value="inscrever">
                    <button type="submit" class="btn btn--primary">Inscrever novamente</button>
                </form>
                <?php else: ?>
            ...
            <?php elseif ($vagas_disp > 0): ?>
                ...
                <p style="color:var(--clr-muted);margin-bottom:16px">
                    Ainda ha <?= $vagas_disp ?> vaga<?= $vagas_disp != 1 ? 's' : '' ?> disponivel<?= $vagas_disp != 1 ? 'eis' : '' ?>.
                </p>
                <form method="POST" action="/pages/inscricao.php">
                    <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                    <input type="hidden" name="evento_id" value="<?= $ev['id'] ?>">
                    <input type="hidden" name="action" value="inscrever">
                    <button type="submit" class="btn btn--primary btn--lg">Inscrever-me gratuitamente</button>
                </form>

            <?php else: ?>
                <div style="text-align:center;padding:16px">
                    <span class="vagas-badge vagas-badge--full" style="font-size:.95rem;padding:8px 20px">Esgotado</span>
                    <p style="margin-top:12px;color:var(--clr-muted)">Nao ha vagas disponiveis para este evento.</p>
                </div>
            <?php endif; ?>

        </div>

        <!-- Acoes do organizador -->
        ...
        <div style="margin-top:24px;display:flex;gap:10px;flex-wrap:wrap;padding:20px;background:var(--clr-bg3);border-radius:12px;">
            <strong style="width:100%;font-size:.82rem;color:var(--clr-muted);text-transform:uppercase;letter-spacing:.05em">Acoes do organizador</strong>
            <a href="/pages/editar_evento.php?id=<?= $ev['id'] ?>" class="btn btn--ghost btn--sm">Editar</a>
            <a href="/pages/presencas.php?id=<?= $ev['id'] ?>" class="btn btn--ghost btn--sm">Lista de presenca</a>
            <a href="/pages/exportar_csv.php?id=<?= $ev['id'] ?>" class="btn btn--ghost btn--sm">Exportar CSV</a>
        </div>
        <?php endif; ?>
    </div>
</div>
