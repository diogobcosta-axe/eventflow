...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_info') {
        ...
        if (strlen($nome) < 2)                          $erros['nome']  = 'Nome muito curto.';
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $erros['email'] = 'Email invalido.';
        ...
        if (empty($erros)) {
            ...
            // Atualiza também os dados na sessão
    ...
    if ($action === 'update_password') {
        ...
        if (!password_verify($atual, $user['password_hash'])) $erros['current_password'] = 'Password atual incorreta.';
        if (strlen($nova) < 6)                                $erros['new_password']      = 'Nova password deve ter pelo menos 6 caracteres.';
        if ($nova !== $confirma)                              $erros['confirm_password']  = 'As passwords nao coincidem.';

        if (empty($erros)) {
            // Cifra a nova password com bcrypt
